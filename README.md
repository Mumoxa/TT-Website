...
- `/downloads` — Public client document library (talenttree.co.za/downloads). Clients can read and download the Terms of Business. The PDF itself is a static file at `/downloads/Talent-Tree-Terms-of-Business.pdf` (source: `public/downloads/`). Linked from the site footer, listed in `public/sitemap.xml`.
